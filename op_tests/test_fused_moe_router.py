...
def test_shared_no_double_count(ep_size, n_shared):
    ...
    g, b, h = _inputs(M, E, dtypes.bf16, 0)
    counts = torch.zeros(M, n_shared, dtype=torch.int64)
    for ep_rank in range(ep_size):
        ...
        want = E + torch.arange(n_shared).view(1, -1)
        assert bool(((tail == want) | (tail == E + n_shared)).all()), tail
        counts += (tail == want).to(torch.int64)
...
def test_shared_bad_args_rejected():
    ...
    def call(**kw):
        kw = dict(n_shared=n_shared, shared_w=1.0, ep_rank=0, ep_size=4, **kw)
        a = kw.pop("outs", got)
        m = kw.pop("mask", mask)
    ...
    with pytest.raises(RuntimeError):
        call(ep_rank=-1)
    # ep_size > 1 with no shared experts is meaningless: nothing to round-robin.
    with pytest.raises(RuntimeError):
        call(n_shared=0)
